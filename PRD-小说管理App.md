...
| AI-002 | AI 摘要生成 | 对已完成抓取的书籍，触发 AI 自动生成小说摘要，包括角色列表和风格标签（如"仙侠、爽文、系统流"），在书籍详情页展示；摘要内容经后端净化：不含推理模型思维链与"（基于样本推断）"类推断性说明字样（v2.6.1） | P1 |
| AI-003 | AI 全网题材推荐 | 全网搜索 tab 输入为题材/自然语言描述且 AI 开关开启时，先由 LLM 推荐 2-3 本该题材知名小说（含一句话推荐理由）以卡片展示，用户选中一本后以书名执行全网源站搜索；可跳过推荐直接搜索，Dify 不可用时自动回退为关键词直搜（v2.6.1） | P1 |
...
- AI 摘要正文仅含【情节摘要】【主要角色】【风格标签】【阅读提示】四段正式内容，不含"（基于样本推断）"等推断性说明与推理模型思维链（v2.6.1）
- 全网 tab 输入题材/描述时先展示 AI 推荐卡片（2-3 本含推荐理由），选中后以书名全网搜索；「跳过推荐，直接搜索」与 Dify 不可用回退均可用（v2.6.1）
- AI 摘要生成中显示 loading 状态，失败时显示重试按钮
- 相关 API：`POST /api/v1/ai/search`、`POST /api/v1/ai/recommend`、`POST /api/v1/ai/summary/{book_id}`、`GET /api/v1/ai/summary/{book_id}`
...
| **v2.6** | 边爬边看（后台异步爬取 + 实时数据推送）：① 后端抓取流水线 v5——单章抓取完成即增量写库（首批写入前才清旧章节，无缝衔接），SSE 事件总线实时推送 `plan`/`chapter_ready`/`done`/`failed`；新增接口 `GET /books/{id}/crawl-stream`（SSE，15s 心跳保活）；② 详情页——轮询改 SSE 订阅（断连自动回退轮询），进度条与目录随抓取实时增长，抓取中即显示「立即阅读」入口；③ 阅读器——抓取中可进入阅读（头部实时徽标），目录合并展示待抓取章节（半透明+「抓取中」标签），点开未就绪章节显示等待态、就绪后自动加载正文；④ 验证：真实源站 149 章 e2e 10/10 通过（`test/e2e_live_crawl_test.py`）+ 浏览器实测通过 | ✅ 已完成 |
| **v2.6.1** | AI 全网题材推荐 + 摘要内容净化：① 全网搜索 tab 输入为题材/描述且 AI 开关开启时先调 `POST /api/v1/ai/recommend`（Dify Chatflow `web-genre-recommend.yml`）展示 2-3 本推荐卡片（含推荐理由），选中后以书名全网搜索，支持跳过推荐与 Dify 不可用回退（新增 searchPage store 状态 + SearchView 推荐面板）；② AI 摘要净化——后端 `_sanitize_summary_text` 剥离推理型模型思维链、移除"（基于样本推断）"等推断性说明，摘要提示词同步改为确定性口吻（`book-summary.yml`）；③ Dify DSL 修正——三个工作流 `mode` 改 `advanced-chat`/节点边 `type: custom`/Code 节点 outputs 字典化，修复导入失败与空白节点 | ✅ 已完成 |
| **v2.7** | 流畅度优化（搜索/排行榜/阅读器"几乎无等待"，基线与预算见 `scripts/PERF_BASELINE.md`）：① 全网搜索流式返回——新增 `GET /api/v1/search/external/stream`（SSE 逐源站推送，首结果 <1s，单源超时收紧 connect 6s/总 10s，总时限 20s），前端流式增量渲染 + 关键词→结果 LRU 缓存 SWR（重搜秒出）+「书架已有」即时匹配条；② 排行榜——榜单接口 `limit` 截断（默认 200）+ 弱 ETag/304（重进零流量），进入榜单后空闲串行预取其余榜单，启动时 DB 缓存回填内存（`ranking_cache` 表，MySQL 需执行 `migrations/add_ranking_cache.sql`）；③ 阅读器——正文段落 `content-visibility: auto` 消除大章上屏长任务，空闲预取窗口 N+3 扩到 N+10；④ 全局——hover/pointerdown 预取路由 chunk（`data-preload-route`），后端启动后台预热（连接池+榜单），封面图 `fetchpriority=low` 让路数据请求；⑤ 回归：`scripts/perf_baseline.py` 重跑全绿（除重启后榜单冷首进依赖待执行迁移，见 ②） | ✅ 已完成 |
