...
>
> **v2.0 / v2.1 前端变更（接口本身未变）**：AI 搜索开关移至用户头像下拉菜单（v2.0，未开启时书架内搜索默认普通搜索；全网搜索始终搜索全部源站；v2.4：开关进一步迁入「我的」页面）；AI 开关开启后书架内搜索按查询意图自动选择搜索方式（v2.1：自然语言描述走 `POST /api/v1/ai/search`，书名/作者走 `GET /api/v1/search`，结果页可一次性手动切换）。相关接口行为均保持不变。
>
